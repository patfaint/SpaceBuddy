...
| `earth_roughness` **or** `earth_specular` | Ocean/land roughness mask — see note below | 8 192 × 4 096 px | `8k_earth_specular_map.tif` |
...
> **Roughness / specular map:** Solar System Scope provides a *specular* map (bright = shiny ocean, dark = matte land), but the PBR material needs a *roughness* map (bright = rough, dark = smooth) — the exact inverse. You have two options:
>
> 1. **Drop-in (recommended):** Add the specular `.tif` to Assets.xcassets as **`earth_specular`**. The code inverts it automatically at load time using Core Image — no manual conversion needed.
> 2. **Pre-inverted:** Invert the image yourself (e.g. *Image → Adjustments → Invert* in Photoshop, or `convert input.tif -negate output.tif` with ImageMagick) and add it as **`earth_roughness`**.
